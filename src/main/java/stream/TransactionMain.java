...
public class TransactionMain {
    public static void main(String[] args) {
        ...
        System.out.println(transactions.parallelStream()
                ...
                .collect(toList()));
        //(2) 交易员都在哪些不同的城市工作过？
        System.out.println(transactions.parallelStream().map(tran -> tran.getTrader().getCity()).collect(toSet()));
        //(3) 查找所有来自于剑桥的交易员，并按姓名排序。
        System.out.println(transactions.parallelStream().filter(tran -> "Cambridge".equals(tran.getTrader().getCity()))
        ...
        System.out.println(transactions.parallelStream()
                ...
                .collect(toList()));
        //(5) 有没有交易员是在米兰工作的？
        System.out.println(transactions.parallelStream().filter(tran -> "Milan".equals(tran.getTrader().getCity())).collect(toList()));
        //(6) 打印生活在剑桥的交易员的所有交易额。
